...
public class PretendantInconnu extends Pretendant{
    ...
    public PretendantInconnu() { }

    @Override
    void rendezVous() {
        System.out.println("Bien joué, rendez-vous avec les Inconnu");
        try {
            Moteur.chargerFinDeJeuxRdv();
        } catch (PileException ex) {
            Logger.getLogger(PretendantInconnu.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    void evolutionRelation(String nbPointsChaine) {
        ...
        //Sinon, appel la méthode rendez-vous
        else {
            this.rendezVous();
        }
    }
